...
def checked_model_path(directory: Path, filename: str) -> Path:
    candidate = (directory / filename).resolve()
    try:
        candidate.relative_to(directory.resolve())
    except ValueError as exception:
        raise SystemExit(
            "The selected model filename is outside the package directory."
        ) from exception
    if candidate.is_file():
        return candidate
    raise SystemExit(f"The selected model file was not found: {candidate}")


def primary_model_path(directory: Path, filename: Optional[str] = None) -> Path:
    if filename:
        return checked_model_path(directory, filename)
    ...
    for metadata_name in ("metadata.yaml", "metadata.yml"):
        ...
        if not match:
            continue
        filename = match.group(1).strip().strip(chr(34) + chr(39))
        try:
            return checked_model_path(directory, filename)
        except SystemExit as exception:
            raise SystemExit(
                f"The primary model filename in {metadata_path} is unavailable. "
                "Rerun with --filename and the documented repository-relative path."
            ) from exception
    ...
    raise SystemExit(
        ...
        "the primary file in metadata.yaml. Rerun with --filename and the documented "
...
def main() -> None:
    ...
    parser.add_argument(
        "--filename",
        help="Repository-relative primary model filename when metadata is missing or stale",
    )
